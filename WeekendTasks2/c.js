...
const caesarCipher = (word, num) => {
  ...
  for (let i=0; i<word.length ; i++){
    ...
    // if (arrLetters.length + num >= arrLetters.length) {
    //   newWord += arrLetters[arrLetters.indexOf(word.charAt(i)) + num - arrLetters.length]
    // } else if (num - 1 < arrLetters.length) {
    //   newWord += arrLetters[arrLetters.indexOf(word.charAt(i)) + num]
    // }
...
//     if (arrString[i].toLowerCase() == arrString[arrString.length-(i+1)].toLowerCase()){
//       count += 1
//     }
//   }
//   if (count >1) {
